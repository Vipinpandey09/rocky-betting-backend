...
function parseScores(teams, scoreArray) {
    let homeScore = 0;
    let awayScore = 0;
...
class CricketService {
    ...
    lastSyncedAt = 0;
    SYNC_INTERVAL = 60_000; // 1-minute cache
    async fetchAllCricAPI(endpoint, offset = 0, pages = 0) {
        if (!KEY) {
    ...
    async syncMatchesToDatabase() {
        ...
        for (const m of uniqueMatches) {
            ...
            const marketStatus = status === "FINISHED" ? "SETTLED" : "OPEN";
            if (!market) {
