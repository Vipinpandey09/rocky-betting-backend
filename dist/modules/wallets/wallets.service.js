...
export class WalletsService {
    ...
    async approveDeposit(depositId) {
        return db.transaction().execute(async (trx) => {
            ...
            if (!deposit)
                throw new AppError("Deposit request not found", 404, "DEPOSIT_NOT_FOUND");
            if (deposit.status !== "PENDING")
                throw new AppError("Deposit is already processed", 400, "ALREADY_PROCESSED");
    ...
    async approveWithdraw(withdrawalId) {
        ...
        if (withdrawal.status !== "PENDING")
            throw new AppError("Withdrawal is already processed", 400, "ALREADY_PROCESSED");
        await db.updateTable("withdrawals").set({ status: "COMPLETED" }).where("id", "=", withdrawalId).execute();
        return { success: true };
    }
    async rejectWithdraw(withdrawalId) {
        return db.transaction().execute(async (trx) => {
