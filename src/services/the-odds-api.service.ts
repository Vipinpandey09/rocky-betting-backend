...
export class TheOddsApiService {
  ...
  private readonly cache = new Map<string, { fetchedAt: number; data: OddsApiEvent[] }>();

  private getKey() {
    return env.THE_ODDS_API_KEY.trim();
  }

  private async fetchEvents(sportKey: string): Promise<OddsApiEvent[]> {
  ...
  async syncMatchesToDatabase(sportSlug: "cricket" | "football" | "volleyball"): Promise<void> {
    ...
    for (const event of events) {
      ...
      let last_ball_started_at: Date | null = null;
      let ball_number = 0;
      ...
      if (event.bookmakers && event.bookmakers.length > 0) {
        ...
        if (h2hMarket && h2hMarket.outcomes.length >= 2) {
          ...
          if (outcome1) back1 = outcome1.price;
          if (outcome2) back2 = outcome2.price;
        }
      }
